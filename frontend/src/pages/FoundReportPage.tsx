...
const WHAT_I_SEE = [
  'A name tag or initials inside',
  'A unique sticker, patch, or marking',
  'A specific piece of damage or wear',
...
export default function FoundReportPage() {
  ...
  const [handoffMethod, setHandoffMethod] = useState<'in_person' | 'email'>('in_person');
  const [marks, setMarks] = useState<MarkState[]>(
    WHAT_I_SEE.map(label => ({ label, checked: false, detail: '' }))
  );
  const [charCount, setCharCount] = useState(0);
  const [confirmed, setConfirmed] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);

  const { register, handleSubmit, formState: { errors } } = useForm<{ message: string; phone?: string; availability?: string }>({
  ...
  function onSubmit(data: { message: string; phone?: string; availability?: string }) {
    if (!confirmed) {
      setSubmitError('Please confirm that you found this item and intend to return it.');
      return;
    }
    setSubmitError(null);
    ...
    <>
      ...
      <main className="laf-shell" style={{ paddingBottom: 60 }}>
        ...
        <header style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#059669', marginBottom: 10, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <p style={{ margin: 0, fontSize: 14.5, color: 'var(--fg-2)', lineHeight: 1.55, maxWidth: 560 }}>
            Your details help the manager match you with the owner and coordinate the handoff safely.
          </p>
        </header>

        {/* Step rail */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
          {['Your details', 'Handoff', 'Review'].map((step, i) => (
            <div key={step} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, fontWeight: 600, color: i === 0 ? 'var(--fg-1)' : 'var(--fg-3)' }}>
                <div style={{
                  ...
                  background: i === 0 ? '#059669' : '#fff',
                  color: i === 0 ? '#fff' : 'var(--fg-3)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 11.5, fontWeight: 700,
                  boxShadow: i === 0 ? '0 4px 12px -4px rgba(5,150,105,0.45)' : undefined,
                }}>
                  {i + 1}
                </div>
                {step}
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border-subtle)', borderRadius: 16, padding: '14px 18px', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 14, boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: 'radial-gradient(circle at 30% 30%, #FEE2E2 0%, #FCA5A5 95%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#059669', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 2 }}>You're reporting a find for</div>
            ...
            {item.locationLabel && (
              <div style={{ fontSize: 12.5, color: 'var(--fg-3)', marginTop: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                Last seen: {item.locationLabel}
              </div>
            )}
          </div>
          <span className="laf-pill laf-pill-lost" style={{ flexShrink: 0 }}>lost</span>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="laf-card" style={{ marginBottom: 20 }}>
              ...
              <p style={{ margin: '0 0 14px', fontSize: 13, color: 'var(--fg-3)', lineHeight: 1.5 }}>
                Describe exactly where you found it, when, and what condition it was in.
              </p>
              <div style={{ position: 'relative' }}>
                <textarea
                  {...register('message', {
                    required: 'Please describe where and when you found the item.',
                    minLength: { value: 10, message: 'Please write at least 10 characters.' },
                    maxLength: { value: 1000, message: 'Maximum 1000 characters.' },
                  })}
                  className="laf-textarea"
                  rows={5}
                  placeholder="e.g. I found this black backpack near the main entrance on Tuesday evening. It was sitting unattended for over an hour. I handed it to the front desk…"
            ...
            {/* What I can see on the item */}
            <div className="laf-card" style={{ marginBottom: 20 }}>
              <h2 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, letterSpacing: '-0.012em', color: 'var(--fg-1)' }}>
                What can you see on the item?
              </h2>
              <p style={{ margin: '0 0 14px', fontSize: 13, color: 'var(--fg-3)', lineHeight: 1.5 }}>
                Check all that you can observe — this helps the manager verify you genuinely have the item.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {marks.map((mark, i) => (
                  <div key={mark.label} style={{ border: `1.5px solid ${mark.checked ? '#059669' : 'var(--border-subtle)'}`, borderRadius: 12, padding: '12px 14px', background: mark.checked ? '#ECFDF5' : '#fff', transition: 'all 140ms' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', userSelect: 'none' }}>
                      <div
                        ...
                        style={{
                          ...
                          background: mark.checked ? '#059669' : '#fff',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                      ...
                      <span style={{ fontSize: 13.5, fontWeight: 500, color: mark.checked ? '#065F46' : 'var(--fg-1)' }} onClick={() => toggleMark(i)}>
                        {mark.label}
                      </span>
                    </label>
                    {mark.checked && (
            ...
            <div className="laf-card" style={{ marginBottom: 20 }}>
              <h2 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, letterSpacing: '-0.012em', color: 'var(--fg-1)' }}>
                Photos of the item you found
              </h2>
              <p style={{ margin: '0 0 14px', fontSize: 13, color: 'var(--fg-3)', lineHeight: 1.5 }}>
                Upload up to 4 photos — front, back, labels, or anything distinctive.
              </p>
            ...
            <div className="laf-card" style={{ marginBottom: 20 }}>
              ...
              <p style={{ margin: '-8px 0 14px', fontSize: 13, color: 'var(--fg-3)', lineHeight: 1.5 }}>
                How would you like to return the item to its owner?
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
                {[
                  ...
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setHandoffMethod(opt.value)}
                    style={{
                      padding: '14px 16px',
                      borderRadius: 14,
                      border: `1.5px solid ${handoffMethod === opt.value ? '#059669' : 'var(--border-subtle)'}`,
                      background: handoffMethod === opt.value ? '#ECFDF5' : '#fff',
                      cursor: 'pointer', textAlign: 'left',
                      transition: 'all 140ms',
                      display: 'flex', alignItems: 'flex-start', gap: 10,
                    }}
                  >
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: handoffMethod === opt.value ? '#059669' : 'var(--slate-100)', color: handoffMethod === opt.value ? '#fff' : 'var(--fg-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ fontSize: 13.5, fontWeight: 600, color: handoffMethod === opt.value ? '#065F46' : 'var(--fg-1)', lineHeight: 1.3 }}>{opt.label}</span>
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div className="laf-field" style={{ margin: 0 }}>
                  <label className="laf-label" htmlFor="availability">
                    When can you hand it off?
                  </label>
                  <select id="availability" className="laf-input" {...register('availability')}>
                    <option value="">Select availability…</option>
                    {HANDOFF_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
                  </select>
                </div>
              </div>
            </div>
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer', marginBottom: 20 }}>
                <div
                  ...
                  style={{
                    ...
                    background: confirmed ? '#059669' : '#fff',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    cursor: 'pointer', transition: 'all 120ms',
                  }}
                >
                ...
                <span style={{ fontSize: 14, color: 'var(--fg-1)', lineHeight: 1.5 }}>
                  I confirm that I found this item and will return it to its rightful owner once the manager verifies the match.
                </span>
              </label>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 18, borderTop: '1px solid var(--border-subtle)' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    ...
                    className="laf-btn"
                    style={{
                      background: confirmed ? '#059669' : 'var(--slate-200)',
                      color: confirmed ? '#fff' : 'var(--fg-3)',
                      border: 'none',
                      opacity: !confirmed ? 0.65 : 1,
                      display: 'flex', alignItems: 'center', gap: 6,
                      padding: '9px 18px', borderRadius: 10, fontWeight: 600, fontSize: 13.5, cursor: confirmed ? 'pointer' : 'default',
                    }}
                  >
                    {mutation.isPending ? 'Submitting…' : 'Submit find report'}
                    {!mutation.isPending && (
          ...
          <aside style={{ position: 'sticky', top: 96, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="laf-card">
              <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--fg-2)' }}>
                How returning a found item works
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {[
                  { n: 1, title: 'Submit your report', desc: 'Tell us where you found it and add any photos or observations.', color: '#059669' },
                  { n: 2, title: 'Manager verifies', desc: 'The space manager matches your report with the person who filed a lost report.', color: '#0891B2' },
                  { n: 3, title: 'Safe handoff', desc: 'Once matched, you\'ll receive instructions to hand the item back safely.', color: 'var(--brand-indigo-600)' },
                ].map(step => (
            ...
            <div style={{ background: 'linear-gradient(135deg, #ECFDF5 0%, #D1FAE5 100%)', border: '1px solid #A7F3D0', borderRadius: 18, padding: '18px 20px', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#065F46' }}>Thank you for your honesty</h3>
              </div>
              <p style={{ margin: 0, fontSize: 13, color: '#065F46', lineHeight: 1.55, opacity: 0.85 }}>
                Your contact details are only shared with the space manager. The owner won't receive your personal info until the manager approves the match.
              </p>
            </div>
          </aside>
        </div>
